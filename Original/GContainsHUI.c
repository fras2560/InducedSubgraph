/* Title: GContainsHUI.c
 * Date: July 19, 2011
 * Author: Daniel Steeves
 * ID: 090565910
 * Email: stee5910@mylaurier.ca
 * Input: Path to graph G (on prompt)
 *        Path to graph H (on prompt)
...
int main()
      ...
      */
{  
  char GGraphText[PATHLENGTH]; /* Path of G file */
  char HGraphText[PATHLENGTH]; /* Path of H file */
  ...
  GGraphText[0] = NULL;
  HGraphText[0] = NULL; /* Empty strings */

  printf("\n");

  printf("Enter the path of the G (containing graph) file:\n");
  fgets(GGraphText, PATHLENGTH, stdin); /* Read the G file path */
  if(GGraphText[strlen(GGraphText) - 1] == '\n')
    /* If the path ends in a newline */
    GGraphText[strlen(GGraphText) - 1] = NULL; /* Kill the newline */

  printf("Enter the path of the H (contained graph) file:\n");
  fgets(HGraphText, PATHLENGTH, stdin); /* Read the H file path */
  if(HGraphText[strlen(HGraphText) - 1] == '\n')
    /* If the path ends in a newline */
    HGraphText[strlen(HGraphText) - 1] = NULL; /* Kill the newline */
